...
class AddChoreViewModel: ObservableObject {
    ...
    func removeTimeStamp(fromDate: Date) -> Date {
        print("____REMOVING TIMESTAMP: ", fromDate)
        guard let date = Calendar.current.date(from: Calendar.current.dateComponents([.year, .month, .day], from: fromDate))
        else {
            fatalError("Failed to strip time from Date object")
        }
        print("______REMOVED TIMESTAMP:", date)
        return date
    }
}
